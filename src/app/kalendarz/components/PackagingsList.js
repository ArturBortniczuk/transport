...
export default function PackagingsList({ onDragEnd }) {
  ...
  const handleDragEnd = (result) => {
    ...
    if (draggedPackaging && onDragEnd) {
      // Przekazujemy opakowanie i identyfikator docelowego dnia
      onDragEnd(draggedPackaging, result.destination.droppableId)
    }
  }

  // Wyciągnij tylko istotne informacje z opisu
  const extractPackagingInfo = (description) => {
    if (!description) return null;
    
    const lines = description.split('\n');
    let result = '';
    
    // Wybierz tylko pierwsze kilka linii, które zawierają istotne informacje
