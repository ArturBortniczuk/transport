   ...
   for (let i = 0; i < allPlacemarks.length; i++) {
     ...
     if (lat && lng) {
       packagings.push({
         ...
         description: description || 'Brak opisu', // Upewnij się, że opis nie jest pusty
         city: city,
...
export async function POST(request) {
 try {
   ...
   const mapId = rawMapId.split('&')[0]; // Pobierz tylko część przed pierwszym &
   ...
   // Podejście jednorazowe - najpierw stwórzmy tabelę na nowo, jeśli to konieczne
   try {
     // Sprawdź czy tabela istnieje a jeśli nie, utwórz ją
     const tableExists = await db.schema.hasTable('packagings');
     console.log('Czy tabela packagings istnieje:', tableExists);
     
     if (!tableExists) {
       console.log('Tworzenie tabeli packagings...');
       ...
       console.log('Tabela packagings została utworzona');
     }
     
     // Alternatywnie, można usunąć istniejącą tabelę i utworzyć ją od nowa (OSTROŻNIE!)
     // await db.schema.dropTableIfExists('packagings');
     // console.log('Usunięto tabelę packagings jeśli istniała');
     // await db.schema.createTable('packagings', table => { ... });
     
   } catch (tableError) {
     console.error('Błąd podczas operacji na tabeli:', tableError);
   }
   
   // Przetwórz każde opakowanie pojedynczo
   console.log('Rozpoczynam dodawanie opakowań pojedynczo...');
   for (let i = 0; i < packagings.length; i++) {
     const packaging = packagings[i];
     try {
       console.log(`Dodawanie opakowania ${i+1}/${packagings.length}: ${packaging.client_name}`);
       ...
       const exists = await db('packagings')
         .where('external_id', packaging.external_id)
         .first();
       
       if (exists) {
         console.log(`Opakowanie z external_id ${packaging.external_id} już istnieje, ID: ${exists.id}`);
         if (exists.status === 'pending') {
           try {
             ...
             console.log(`Zaktualizowano opakowanie ID ${exists.id}`);
             importResults.updated++;
           } catch (updateError) {
             console.error(`Błąd aktualizacji opakowania ID ${exists.id}:`, updateError);
             importResults.errors++;
           }
         }
       } else {
         // Prostszy insert z pojedynczymi polami
         try {
           const result = await db('packagings').insert({
             external_id: packaging.external_id,
             client_name: packaging.client_name || 'Bez nazwy',
             description: packaging.description || 'Brak opisu',
             city: packaging.city || 'Nieznane',
             postal_code: packaging.postal_code || '',
             street: packaging.street || '',
             latitude: packaging.latitude,
             longitude: packaging.longitude,
           ...
           console.log(`Dodano nowe opakowanie, wynik operacji:`, result);
           importResults.added++;
         } catch (insertError) {
           console.error(`Błąd podczas dodawania opakowania ${packaging.client_name}:`, insertError);
           console.error('Szczegóły błędu:', insertError.stack);
           importResults.errors++;
         }
       }
     } catch (error) {
       console.error(`Ogólny błąd dla opakowania ${i+1}:`, error);
       importResults.errors++;
     }
   }
   ...
   // Na końcu funkcji POST, przed zwróceniem odpowiedzi
   console.log('----------------------');
   console.log('Podsumowanie synchronizacji:');
   console.log(`Dodano: ${importResults.added}, Zaktualizowano: ${importResults.updated}, Błędy: ${importResults.errors}`);
   console.log('----------------------');
